...
class AddSimCardScreen extends ConsumerStatefulWidget {
  const AddSimCardScreen({super.key});

  @override
  ConsumerState<AddSimCardScreen> createState() => _AddSimCardScreenState();
}

class _AddSimCardScreenState extends ConsumerState<AddSimCardScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    final success = await ref.read(providers.simCardNotifierProvider.notifier).createSimCardWithParams(
      userId: userId,
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('SIM card added successfully'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(simCardState.error ?? 'Failed to add SIM card'),
            backgroundColor: Colors.red,
          ),
        );
      }
